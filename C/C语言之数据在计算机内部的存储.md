...
对于正数说，因为原、反、补都是相同的~~  
![](./C语言之数据在计算机内部的存储.assets/1739441071466-6ffff3ee-6834-429f-8dfa-80450dc9dc31.png)
...
![](./C语言之数据在计算机内部的存储.assets/1739441072797-f3ec98a8-d7f4-49d4-b0ae-86a958d7eb24.png)

#### 1.2 原码与补码的转换形式
![](./C语言之数据在计算机内部的存储.assets/1739440554095-ee473bc6-3807-4c39-ac9c-ee4d4131103a.png)

![](./C语言之数据在计算机内部的存储.assets/1739441075163-25b24cc3-507c-46ba-9e21-34aece6bb920.png)

#### 1.3 计算机内部的存储编码
+ 对于整形来说：数据存放内存中存放的是补码。

![](./C语言之数据在计算机内部的存储.assets/1739441077036-9a607296-42e9-40e0-8aec-8b26b0f52971.png)
...
![](./C语言之数据在计算机内部的存储.assets/1739441078786-556849d9-6d24-4882-8ab7-fa01470db33d.png)

+ 【大端字节序存储】是要将高位存放到低地址，低位存放到高地址，因此11要放在左边，44要放在右边

![](./C语言之数据在计算机内部的存储.assets/1739441079913-a900fd44-5263-473e-989c-959ce3e500f4.png)
...
![](./C语言之数据在计算机内部的存储.assets/1739441081738-1639668a-da65-472b-88f2-28ef6dda759b.png)
...
![](./C语言之数据在计算机内部的存储.assets/1739441083739-102c28dd-e170-4ac3-b494-1d578250b3e6.png)
...
![](./C语言之数据在计算机内部的存储.assets/1739441085447-f8e9749e-91c4-495f-bbbe-c26881474c30.png)
...
+ 首先我们通过下面这幅图来看一看对于有符号的`char`和无符号的`char`在内存中所能表示的范围各自是多少：
    - 【signed char】：`-128 ~ 127`
    - 【unsigned char】：`0 ~ 255`

![](./C语言之数据在计算机内部的存储.assets/1739441087790-522e6cc2-88fc-4f50-a0d6-724b26e6658c.png)
...
+ ![](./C语言之数据在计算机内部的存储.assets/1739441090126-d08abcea-bf84-4b32-93df-b17df61503dd.png)
...
![](./C语言之数据在计算机内部的存储.assets/1739441108476-b844f67f-ad33-4c71-aef0-6d0d3b05f9bd.png)

![](./C语言之数据在计算机内部的存储.assets/1739441109640-81ad95b1-93cd-48c0-98a6-141a1ecd7b52.png)
...
![](./C语言之数据在计算机内部的存储.assets/1739441113445-5908a437-cd2b-45b8-ae8d-4f33362c977e.png)
...
![](./C语言之数据在计算机内部的存储.assets/1739441115989-8b1e5012-a26f-443f-8f2b-94f7c4a286ea.png)
...
![](./C语言之数据在计算机内部的存储.assets/1739440828335-35107a9f-ae97-47e8-8473-bb41236cadf6.png)
...
![](./C语言之数据在计算机内部的存储.assets/1739441119942-c595071c-2b86-49f9-88a9-9b45eae54d2d.png)
...
![](./C语言之数据在计算机内部的存储.assets/1739441121443-495366fa-75c3-4e68-a65e-2b2620ebe920.png)
...
![](./C语言之数据在计算机内部的存储.assets/1739441123803-53b9eb72-fc31-4aba-a047-3553c0711836.png)
...
![image](./C语言之数据在计算机内部的存储.assets/2ff2f88f5e5c5376013124d3ec1ef691.svg)
...
1. **v = 5.5**
    ...
    - 接着我们就要去求出S、M和E，对于M来说是>= 1并且< 2的，不过这里的`101.1`却远远大于1，所以我们可以通过在操作符中学习的【移位】操作将这个数进行左移两位，但是左移之后又要保持与原来的数相同，所以可以再乘上*_22__*__使得与原来的二进制相同。接着根据公式就可以写出__*__(-1)0 __*__ 1.011 __*__ 22这个式子_*，然后可以得出S、M、E为多少了
2. **v = -5.5**
...
![](./C语言之数据在计算机内部的存储.assets/1739441012335-9bae2b31-60db-4927-8c99-9a8ad091a488.png)

+ 对于`64位`的浮点数【double】，最高的1位是符号位`S`，接着的11位是指数`E`，剩下的52位为有效数字`M`

![](./C语言之数据在计算机内部的存储.assets/1739441023157-82395032-a40f-49ed-8b24-a7cd8d3f76b1.png)

**其中S代表符号位，E代表阶码，M代表尾数**
...
+ 实际上规格化单精度浮点数的阶码e的取值范围是[1, 254]，双精度取值范围是[1, 2046]，这是因为全0和全1已经被特殊规定了

科学计数法中的**E是可以出现负数**的，所以IEEE 754规定，存入内存时E的真实值必须再加上一个**中间数**，**对于8位的E，这个中间数是127；对于11位的E，这个中间数是1023**
...
+ 首先写出【n = 9】在内存中的补码`0 0000000 00000000 00000000 00001001`。然后是将这个n的地址存放到了一个浮点型的指针中
