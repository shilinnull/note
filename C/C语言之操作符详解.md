...
![](./C语言之操作符详解.assets/1739437303790-46b069ec-7734-47d0-855c-958affe2f8a4.png)

浮点数除法：

![](./C语言之操作符详解.assets/1739437337727-126ad51b-b331-45bd-85ac-d3dd0acd13cd.png)

取余操作符

![](./C语言之操作符详解.assets/1739437282754-967ea9b3-b20f-4227-9fb3-fc086f7d98e1.png)
...
![](./C语言之操作符详解.assets/1739438938155-be6371ba-e8c4-46bf-b403-d7c465ffbd2b.png)

+ 从上图也可以看出，执行**左边丢弃，右边补0**，也就是相当于`乘2`，结果就是`8`

![](./C语言之操作符详解.assets/1739438939580-2318fab2-45b3-4973-b470-735394f484fb.png)
...
![](./C语言之操作符详解.assets/1739438941317-8a00241f-7898-4378-9d9c-46dd583ca231.png)

+ 我们再将移位后的结果再次变成原码的操作，也就是 **-1再取反** 即可变成原码
+ 最后得出的结果再转换为十进制便是`8`

![](./C语言之操作符详解.assets/1739438942347-9dc338f2-6307-4272-8070-f40ea10c65d5.png)
...
![](./C语言之操作符详解.assets/1739438943352-83b05b73-3e80-4b1e-a0b7-5b76e0541fa7.png)

+ 对于右移从结果来看就相当于是缩小的形式

![](./C语言之操作符详解.assets/1739438944686-a74058a0-1fa9-4ab4-a695-eda09f35562b.png)
...
![](./C语言之操作符详解.assets/1739438945971-5b599f45-47e1-4624-a8ca-ca8ce6f35db8.png)

![](./C语言之操作符详解.assets/1739438947301-e7fef8bf-65ff-41fa-a9df-44661b9f0950.png)
...
![](./C语言之操作符详解.assets/1739438952361-410bef73-4acb-402a-b79c-c97e79ba1c01.png)
...
![](./C语言之操作符详解.assets/1739438954224-2914445f-e8b3-4156-b795-9450e6a6e5ae.png)
...
![](./C语言之操作符详解.assets/1739438956784-5056a0f0-35e6-47d4-b0cd-d8cf2f84ddfe.png)
...
![](./C语言之操作符详解.assets/1739438958369-370e7d58-1bf6-4fac-ae47-5b83890dc7fa.png)
...
![](./C语言之操作符详解.assets/1739438960046-39c14238-1a50-4596-9c4a-1b39444c6e8e.png)
...
![](./C语言之操作符详解.assets/1739438961644-824818bf-0d86-4860-9f06-e8c1036905a4.png)
...
![](./C语言之操作符详解.assets/1739438963161-5f99f81a-27b8-42fa-99f2-54571b85f587.png)
...
![](./C语言之操作符详解.assets/1739438964595-55d2018c-8737-4b2f-b5be-291968352e61.png)
...
![](./C语言之操作符详解.assets/1739438966767-e5dbf859-311b-434e-ad45-49501cebc4c2.png)
...
![](./C语言之操作符详解.assets/1739438968436-2bedff1d-c85a-4e8e-aaed-351c1da85bf0.png)
...
![](./C语言之操作符详解.assets/1739439006634-0d7fda48-c40f-436a-b45d-f80ec649617a.png)
...
![](./C语言之操作符详解.assets/1739439008144-d44d0fab-0b2f-4ee5-add4-df7300de308f.png)

![](./C语言之操作符详解.assets/1739439009918-a275dea1-6dd0-4599-8466-da3ebf35c64b.png)
...
![](./C语言之操作符详解.assets/1739439038064-0932e46e-f2dc-42b5-82b0-30a01fcd3922.png)

### 3、【&】和【*】
+ 【&】来说叫做取地址操作符，可以获取一个变量在内存中的地址

![](./C语言之操作符详解.assets/1739439039399-da3bed05-d79e-4626-96fa-885872839cf5.png)

+ 我们还可以将其给到一个指针变量

![](./C语言之操作符详解.assets/1739439040844-a9a5ec10-cd59-41a7-a2e6-e9f526f454aa.png)

+ 那这个时候我要获取到这个指针所指向的地址就是要用到【*】这个解引用操作符

![](./C语言之操作符详解.assets/1739439042183-16540249-695e-4d1f-b263-0b139b0533cb.png)

+ 这一步操作就是在使这个指针变量重新指向一块新的地址，即`00000014`，转换为十进制也就是【20】

![](./C语言之操作符详解.assets/1739439043338-80569da2-f403-4300-b828-96704a5c4524.png)

### 4、【-】和【+】
+ 对于这两个操作符并不常用，不能完全说只是单目操作符，在特定的场景下它们也可以算是一个`双目操作符`，例如：`-6`的话就只一个单目操作符，`8 - 3`的话就是第一个双目操作符，【+】的话也是同理

### 5、sizeof
对于sizeof来说，这个操作符是用来计算数据类型的大小的，以字节为单位
...
![](./C语言之操作符详解.assets/1739439045548-20f11357-6ec8-495a-9560-dd622789adc8.png)

**sizeof后可省略**

+ sizeof是一个操作符，而不是函数，函数后面必须带一个括号，而操作符可以不带，下面我们就可以验证一下，后面的（）其实是可以省略的

![](./C语言之操作符详解.assets/1739439046885-81de5c6a-e558-46a4-8da7-b7664c55da33.png)
...
![](./C语言之操作符详解.assets/1739439048983-ddc92226-cd78-4828-a9f4-fcba4f95ed71.png)
...
![](./C语言之操作符详解.assets/1739439050525-3766ab46-3495-481a-b239-f545c8155d10.png)
...
![](./C语言之操作符详解.assets/1739439052674-e98e47f8-149a-46bc-8b5a-4d98b2184db0.png)
...
+ 这里的`b = ++a`相当于就是先让a++，然后再把a的值给到b
+ 这里的`b = a++`相当于就是先把a的值给到b，然后再让a++

![](./C语言之操作符详解.assets/1739439060175-6ce62f2f-8d04-43b9-9bfe-b7853a01b44c.png)

![](./C语言之操作符详解.assets/1739439059375-24962cc5-bfa8-430f-8a10-e7af4e6f8f47.png)
...
![](./C语言之操作符详解.assets/1739439062572-9c91d690-08ae-4706-a3ff-7fea0c4c2d99.png)

+ 对于这里的`b = a--`相当于就是先把a的值给到b，然后再让a- -

![](./C语言之操作符详解.assets/1739439063989-e6ac7153-580d-43c4-a1c0-992d38e116f9.png)

### 7、强制类型转换
+ 如果我需要将一个浮点类型的数据赋值给一个整形，编译器会报一个警告【精度丢失】，若要强制赋值，就需要做一个强制类型转换

![](./C语言之操作符详解.assets/1739439065473-fbab1865-f8b3-44d5-984e-21163a51e3cb.png)
...
![](./C语言之操作符详解.assets/1739439066824-0c8bdc89-66f0-4381-b5d2-c3c313851e8e.png)
...
![](./C语言之操作符详解.assets/1739439069001-37735f3a-76b8-4a46-92bf-d79afcddd78b.png)
...
![](./C语言之操作符详解.assets/1739439072940-b46d1688-695a-4724-a03e-dfd60395a04e.png)
...
![](./C语言之操作符详解.assets/1739439076384-61ae2a02-fc3b-4ada-b884-b361ab07e39e.png)
...
![](./C语言之操作符详解.assets/1739438592805-50ba198e-37fb-427a-9d50-e798c7d9f3ec.png)
...
![](./C语言之操作符详解.assets/1739438711515-ff2cf3c8-e93f-40df-af1c-91f9c4ac86a0.png)

## 十一、表达式求值
### 1、隐式类型转换【整型提升】
+ C的整型算术运算总是至少以缺省整型类型的精度来进行的，为了获得这个精度，表达式中的**字符型**和**短整型**操作数在使用之前被转换为普通整型，这种转换称为`[整型提升]`
...
![](./C语言之操作符详解.assets/1739439082364-ed67ec5c-b1ce-46ac-a832-58442689e2c2.png)
...
![](./C语言之操作符详解.assets/1739439084055-30b42438-d0a0-410f-b1cf-88aa99857c52.png)
...
![](./C语言之操作符详解.assets/1739439103449-78253c5b-7fc1-46a0-9412-4421ddeeec82.png)

+ 可以看到，虽然在【VS】和【Linux】在执行的结果是`-10`，而且在大多数的编译器下都是这个，但是呢对于函数的调用先后顺序无法通过操作符的优先级确定，因此这也是一个问题表达式

![](./C语言之操作符详解.assets/1739439105481-a7bce08d-befb-4b52-8376-fcba5b419690.png)
...
![](./C语言之操作符详解.assets/1739439107509-664616ca-f2c9-495d-9f75-55e5280f232d.png)

+ 可以看到，这里就出现了两个不同的结果，VS里运行出来是【12】，在linux里面运行出来是【10】

![](./C语言之操作符详解.assets/1739439108760-17c5cca0-ee49-4abf-b4a9-19811b2de7ca.png)
