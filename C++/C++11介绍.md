...
[initializer_list - C++ Reference](https://legacy.cplusplus.com/reference/initializer_list/initializer_list/)
...
![](./C++11介绍.assets/1745469012110-60cafc52-abec-4dab-a765-65a4a5809a0e.png)
...
![](./C++11介绍.assets/1745501081082-1b431da9-1569-4283-944f-eeed39b60ee6.png)
...
+ [http://www.cplusplus.com/reference/map/map/emplace/](http://www.cplusplus.com/reference/map/map/emplace/)

# 五、右值引用和移动语义
## 5.1 左值引用和右值引用
传统的C++语法中就有引用的语法，而C++11中新增了的右值引用语法特性，我们之前学习的引用就叫做**左值引用**。无论左值引用还是右值引用，都是**给对象取别名**。

> **什么是左值？什么是左值引用？**
>

左值是一个表示数据的表达式(如变量名或解引用的指针)，我们可以获取它的地址+可以对它赋值，左值可以出现赋值符号的左边，右值不能出现在赋值符号左边。定义时const修饰符后的左值，不能给他赋值，但是**可以取它的地址**。左值引用就是给左值的引用，给左值取别名。
...
+ 需要注意的是**右值是不能取地址**的，但是给右值取别名后，会导致右值被存储到特定位置，且可以取到该位置的地址，也就是说例如：**不能取字面量10的地址**，**但是rr1引用后，可以对rr1取地址，也可以修改rr1**。**如果不想rr1被修改，可以用**`**const int&& rr1**`**去引用**。
...
![](./C++11介绍.assets/1745501080805-00ba16db-0664-41aa-80b2-5920ad4270d7.png)
...
移动构造函数是一种构造函数，类似拷贝构造函数，移动构造函数**要求第一个参数是该类类型的引用**，但是不同的是要求这个参数是右值引用，如果还有其他参数，额外的参数必须有缺省值。
...
+ 在`lsl::string`中增加移动构造，移动构造本质是将参数右值的资源窃取过来，占位已有，那么就不用做深拷贝了，所以它叫做**移动构造**，也就是**窃取别人的资源来构造自己**。
...
左边为不优化的情况下，两次拷贝构造，右边为编译器优化的场景下连续步骤中的拷贝合二为一变为一次拷贝构造

> 在linux下使用`g++ test.cpp -fno-elide-constructors -std=c++11`关闭优化就可以看到
>

![](./C++11介绍.assets/1745490191168-f234f4d2-563e-4283-a902-d3a7f645c584.png)

![](./C++11介绍.assets/1745491197887-a4ad79b8-c17e-404f-8502-c65a20177b27.png)

![](./C++11介绍.assets/1745490980437-deea94e9-25bd-4bcb-b163-516152c96745.png)

### 5.2.2 右值对象构造，有拷贝构造，也有移动构造的场景
左边为不优化的情况下，两次移动构造，右边为编译器优化的场景下连续步骤中的拷贝合二为一变为一次移动构造。

![](./C++11介绍.assets/1745490566787-e288415f-a82b-4ee8-bdc6-93afc1982dbc.png)

![](./C++11介绍.assets/1745491214417-0be77ee1-7f3f-4dbd-b404-fcfc0449c949.png)

![](./C++11介绍.assets/1745490991179-d16a0bc5-b045-4c7d-8540-0bfcebf7d7aa.png)

需要注意的是在vs2019的release和vs2022的debug和release，下面代码优化为非常恐怖，会直接将str对象的构造，str拷贝构造临时对象，临时对象拷贝构造ret对象，合三为一，变为直接构造。

![](./C++11介绍.assets/1745491385193-a745937f-b6a8-4eb2-b5ee-e7fc66fe5544.png)

![](./C++11介绍.assets/1745491313576-bb0de2d3-b7e3-48fc-a3b1-ec61e9cd5276.png)

我们可以打印一下str和ret的地址来看一下：

![](./C++11介绍.assets/1745491741393-7ab2f68d-f6f9-45ab-8ee6-6e076e3110e8.png)

### 5.2.3 右值对象赋值，只有拷贝构造和拷贝赋值，没有移动构造和移动赋值的场景
```cpp
int main()
{
    lsl::string ret;
...
![](./C++11介绍.assets/1745492334476-251e6fda-23d4-40b6-b4cc-4c3cdfffa1fb.png)

![](./C++11介绍.assets/1745492377255-1f90e2f6-a92a-4a5e-a8f1-04284ecc2a61.png)

### 5.2.4 右值对象赋值，既有拷贝构造和拷贝赋值，也有移动构造和移动赋值的场景
![](./C++11介绍.assets/1745492627129-5a5ec2db-be6a-4578-a189-3d774cd39bec.png)

![](./C++11介绍.assets/1745492473438-5cd4b2b4-9087-4f9d-9985-797ce7ae4eec.png)

总结：

+ 传值返回的场景，没有移动构造移动赋值，依赖编译器的优化，拷贝构造接收场景效率很好，赋值接收，有一定拷贝代价
+ 传值返回的场景，有移动构造移动赋值，不依赖编译器的优化，无论如何拷贝接收还是赋值接收，代价都很低。
...
![](./C++11介绍.assets/1745474092965-6b894276-8761-49c7-bc03-d144f671ae51.png)

 STL容器插入接口函数也增加了右值引用版本： 

+ [https://legacy.cplusplus.com/reference/list/list/push_back/](https://legacy.cplusplus.com/reference/list/list/push_back/)
+ [https://legacy.cplusplus.com/reference/vector/vector/push_back/](https://legacy.cplusplus.com/reference/vector/vector/push_back/)
...
![](./C++11介绍.assets/1745501080969-93a63420-aef6-4da9-a328-cec24fdaf75a.png)
...
+ 将亡值是指返回右值引用的函数的调用表达式和转换为右值引用的转换函数的调用表达，如move(x)、static_cast<X&&>(x)泛左值(generalized value，简称glvalue)，泛左值包含将亡值和左值。
...
+ 但是引用类型的唯一作用就是限制了接收的类型，后续使用中都退化成了左值(引用折叠）
...
![](./C++11介绍.assets/1745494084206-39b3e705-e921-4ed5-bfb3-ebf1b7281632.png)
...
![](./C++11介绍.assets/1745501080832-7c264fcc-ff36-4733-a743-0bb02cccda70.png)
...
![](./C++11介绍.assets/1745501080940-434152b1-5771-4832-9f5c-e65fdd8b4fcb.png)
...
## 8.1 移动构造函数和移动赋值运算符重载
+ C++11 新增了两个：**移动构造函数**和**移动赋值运算符重载**。

针对移动构造函数和移动赋值运算符重载有一些需要注意的点如下：

+ 如果你没有自己实现移动构造函数，且没有实现**析构函数 、拷贝构造、拷贝赋值重载**中的任意一个。那么编译器会自动生成一个默认移动构造。默认生成的移动构造函数，对于内置类型成员会执行逐成员按字节拷贝，自定义类型成员，则需要看这个成员是否实现移动构造，如果实现了就调用移动构造，没有实现就调用拷贝构造。
...
![](./C++11介绍.assets/1745501081204-acd4a7fc-0860-4c5c-894c-39af0cb4e21b.png)
...
![](./C++11介绍.assets/1745501081212-4e590be5-bb4c-4e0b-abef-92a1788d2b18.png)
...
![](./C++11介绍.assets/1745501081483-8691ccde-6b08-4490-be4a-3d3221e68c3c.png)
...
上面的参数args前面有省略号，所以它就是一个可变模版参数，我们把带省略号的参数称为“参数包”，它里面包含了0到N个模版参数。我们无法直接获取参数包args中的每个参数的，只能通过展开参数包的方式来获取参数包中的每个参数，这是使用可变模版参数的一个主要特点，也是最大的难点，即如何展开可变模版参数。由于语法不支持使用args[i]这样方式获取可变参数，所以我们的用一些奇招来一一获取**参数包**的值。
...
![](./C++11介绍.assets/1745494812997-c6b11c88-27c8-4cac-94d6-99bfa3ae7b83.png)
...
原理2：更本质去看没有可变参数模板，我们实现出这样的多个函数模板才能支持，这里的功能，有了可变参数模板，我们进一步被解放，他是类型泛化基础，上叠加数量变化，让我们泛型编程更灵活。
...
![](./C++11介绍.assets/1745495185962-b7e1a742-16e3-4ca0-b8fd-88e347faef74.png)
...
![](./C++11介绍.assets/1745495592589-11cb7501-b81a-42ec-be14-e5be73830f62.png)
...
emplace_back总体而言是更高效，推荐以后使用emplace系列替代insert和push系列第二个程序中我们模拟实现了list的emplace和emplace_back接口，这里把参数包不段往下传递，最终在结点的构造中直接去匹配容器存储的数据类型T的构造，所以达到了前面说的empalce支持直接插入构造T对象的参数，这样有些场景会更高效一些，可以直接在容器空间上构造T对象。

传递参数包过程中，如果是Args&&... args 的参数包，要用完美转发参数包，方式如下`std::forward<Args>(args)...` ，否则编译时包扩展后右值引用变量表达式就变成了左值。
...
![](./C++11介绍.assets/1745501081501-d1626c2c-b0b4-4e5d-9826-f190e172631f.png)
...
![](./C++11介绍.assets/1745501081635-069f2ef7-84df-4a9a-8ad5-a7f68fc3ce1a.png)
...
+ `{function body}`：函数体，在该函数体内，除了可以使用其参数外，还可以使用所有捕获到的变量，函数体为空也不能省略
...
![](./C++11介绍.assets/1745501081667-b06830a1-788d-490a-8ade-d78f688eba18.png)

+ 名字是`lambda_uuid`

![](./C++11介绍.assets/1745501081700-10323317-b6f2-4736-b295-75153010966d.png)
...
![](./C++11介绍.assets/1745501081923-8b6ec094-0559-4451-b2ae-59c33669bd2a.png)
...
![](./C++11介绍.assets/1745501081958-c900e776-9a34-4a57-b1c1-07bc05609d25.png)
...
![](./C++11介绍.assets/1745501082220-372e04c4-d85d-47d6-b0a3-871fdfb1066b.png)
...
## 11.1 包装器的使用场景
[150. 逆波兰表达式求值 - 力扣（LeetCode）](https://leetcode-cn.com/problems/evaluate-reverse-polish-notation/submissions/)
...
[bind](https://legacy.cplusplus.com/reference/functional/bind/?kw=bind)

![](./C++11介绍.assets/1745501082194-d8dd1fe8-7724-45be-be07-97980263fc08.png)
