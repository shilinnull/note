...
一个服务器可能被多个客户端来连接，每一个都是对应一个文件描述符对外提供服务的， 未来我们一旦建立好连接，服务器不能用创建好的文件描述符和客户端通信，而应该让accept的返回值文件描述符来给用户提供服务。
...
新线程创建好了，主线程也要回收新线程的资源。以前用的是`pthread_join`，但是在后面我们学过可以使用`pthread_deatch`进行线程分离，主线程就不用等了。
