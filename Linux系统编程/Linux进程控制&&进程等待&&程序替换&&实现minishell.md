...
+ exit是库函数，\_exit是系统调用函数，而库函数内部封装了系统调用。 也就是说，调用exit函数最终也会调用\_exit来使进程退出，只不过在其调用_exit之前，还做了其他工作：
...
+ status是一个int类型的值，意味着它应该有32个比特位，但它又不能被当初普通的整形来看待，因为其**高16位的值并不被使用**，而**只使用其低16个比特位**：
...
+ 其中`l`和`v`的区别在于**程序运行参数的赋予方式不同**，**`l`是通过函数参数逐个给与**，最终以**NULL结尾**，**而v是通过字符指针数组一次性给与**。
