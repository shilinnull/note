...
3. 管道一直在读 && **写端关闭了wfd**，读端**read返回值会读到0**，表示**读到了文件末尾。**
4. **读端rfd**直接关闭，但是**写端wfd**一直在进行写入，写端进程会被操作系统直接使用**13信号**关掉，**相当于进程出现了异常。**
...
4. 第一次创建的时候，必<font style="color:#DF2A3F;"></font>须有一个key，怎么有？（一会谈）
